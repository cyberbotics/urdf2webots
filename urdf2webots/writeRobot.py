...
def header(robotFile, srcFile=None, tags=[]):
    ...
    if robotName:
        robotFile.write('# This is a robot ' + robotName + ' converted from an URDF file.\n')
    if header.sourceFile is not None:
        robotFile.write('# Extracted from: ' + header.sourceFile + '\n')
...
def URDFLink(robotFile, link, level, parentList,
    ...
    if not dummy:  # dummy: case when link not defined but referenced (e.g. Atlas robot)
        ...
        if haveChild:
            ...
            if requiredName:
                robotFile.write((level + 1) * indent + 'name "' + requiredName + '"\n')
        else:
    ...
    for visualNode in link.visual:
        ...
        if enableMultiFile and visualNode.geometry.trimesh.coord:
            ...
            if visualNode.geometry.defName is None:
                print('Create meshFile: %sMesh.robotFile' % name)
                filepath = '%s/%sMesh.robotFile' % (meshFilesPath, name)
                meshrobotFileFile = open(filepath, 'w')
                header(meshrobotFileFile, tags=['hidden'])
                meshrobotFileFile.write('robotFile %sMesh [\n]\n{\n' % name)
                visualNode.material.defName = None
                URDFVisual(meshrobotFileFile, visualNode, 1, normal)
                meshrobotFileFile.write('}\n')
                meshrobotFileFile.close()
